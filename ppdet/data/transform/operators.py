...
class YOLOv5KeepRatioResize(BaseOperator):
    ...
    def __init__(self, target_size, keep_ratio=True, batch_shapes=True, size_divisor=32, extra_pad_ratio=0.5):
    ...
    def apply_image(self, image):
        ...
        ratio = self._get_rescale_ratio((original_h, original_w), self.target_size)
        if ratio != 1:
    ...
    def apply(self, sample, context=None):
        ...
        im, (resize_h, resize_w), (scale_ratio_h, scale_ratio_w) = self.apply_image(sample['image'])
        # (427, 640) (480, 640)
        sample['image'] = im.astype(np.float32)
        sample['im_shape'] = np.asarray([resize_h, resize_w], dtype=np.float32)
        sample['scale_factor'] = np.asarray([scale_ratio_h, scale_ratio_w], dtype=np.float32)

        shapes = [[1, 1]]
        aspect_ratio = resize_h / resize_w
        shapes = [aspect_ratio, 1] if aspect_ratio < 1 else [1, 1 / aspect_ratio]
...
class LetterResize(BaseOperator):
    ...
    def _resize_img(self, results):
        ...
        if top_padding != 0 or bottom_padding != 0 or \
            ...
            if isinstance(self.pad_val, int) and image.ndim == 3:
                self.pad_val = tuple(self.pad_val for _ in range(image.shape[2]))
            # image = cv2.impad(
            #     img=image,
            ...
            image = cv2.copyMakeBorder(
                image, top, bottom, left, right, cv2.BORDER_CONSTANT, value=self.pad_val)
            # (448, 672, 3) 
    ...
    def apply(self, sample, context=None):
        ...
        if 'scale_factor_origin' in sample:
            ...
            scale_ratio_h, scale_ratio_w = (sample['scale_factor'][0] *
