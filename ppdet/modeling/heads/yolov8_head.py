...
__all__ = ['YOLOv8Head', 'YOLOv8Head_rect']
...
class YOLOv8Head(nn.Layer):
    ...
    def forward_eval(self, feats):
        ...
        return feats_shapes, cls_logits_list, bbox_preds_list, anchor_points, stride_tensor
    ...
    def get_loss(self, head_outs, gt_meta):
        ...
        pred_bboxes = batch_distance2bbox(anchor_points_s, pred_distri) # xyxy [8, 8400, 4]
    ...
    def post_process(self, head_outs, im_shape, scale_factor):
        feats_shapes, pred_scores_list, pred_dist_list, anchor_points, stride_tensor = head_outs
        pred_scores = [
            cls_score.transpose([0, 2, 3, 1]).reshape([-1, size, self.num_classes])
            for size, cls_score in zip(feats_shapes, pred_scores_list)
        ]
        pred_dists = [
            bbox_pred.transpose([0, 2, 3, 1]).reshape([-1, size, 4])
            for size, bbox_pred in zip(feats_shapes, pred_dist_list)
        ]
...
@register
class YOLOv8Head_rect(nn.Layer):
    ...
    def __init__(self,
                 ...
                 exclude_post_process=False,
                 print_l1_loss=True):
        super(YOLOv8Head_rect, self).__init__()
        assert len(in_channels) > 0, "len(in_channels) should > 0"
        self.in_channels = in_channels
        self.num_classes = num_classes
        self.fpn_strides = fpn_strides
    ...
    def forward_eval(self, feats):
        ...
        for i, feat in enumerate(feats):
            ...
            bbox_dist_preds = bbox_dist_preds.reshape(
                [-1, 4, self.reg_max, l]).transpose(
                    [0, 3, 1, 2])  # [8, 6400, 4, 16]
            bbox_preds = F.softmax(
                bbox_dist_preds,
                axis=3).matmul(self.proj.reshape([-1, 1])).squeeze(
                    -1)  # [8, 6400, 4]
            cls_logits_list.append(cls_logit)
    ...
    def post_process(self, head_outs, img_meta, rescale=1):
        feats_shapes, pred_scores_list, pred_dist_list, anchor_points, stride_tensor = head_outs
        pred_scores = [
        ...
        pred_scores = F.sigmoid(paddle.concat(pred_scores, 1))[0]  # [8, 8400, 80]
        pred_bboxes = paddle.concat(pred_dists, 1)  # [8, 8400, 4]
        ...
        nms_pre = self.nms.nms_top_k  # 3000

        # print('pred_scores', pred_scores.shape)
        # print('pred_bboxes', pred_bboxes.shape)

        if self.multi_label is False:
            labels = pred_scores.argmax(1, keepdim=True)
            scores = pred_scores.max(1, keepdim=True)
            ...
            pred_scores, pred_labels, keep_idxs, _ = filter_scores_and_topk(
                pred_scores, score_thr, nms_pre)
        pred_bboxes = pred_bboxes[keep_idxs]  # [2631, 4]
        ...
        if rescale:
            ...
            if pad_param is not None:
                pred_bboxes -= paddle.to_tensor([
                    int(pad_param[2]), int(pad_param[0]), int(pad_param[2]),
                    int(pad_param[0])
                ])

            scale_y, scale_x = paddle.split(
                img_meta['scale_factor'][0], 2, axis=-1)
            scale_factor = paddle.concat([scale_x, scale_y, scale_x, scale_y],
                                         -1)
            pred_bboxes /= scale_factor
